...
const STATUS_COLORS = {
  paid: '#10B981', pending: '#F59E0B', authorized: '#3B82F6',
  refunded: '#EF4444', voided: '#6B7280', partially_refunded: '#8B5CF6',
  partially_paid: '#F59E0B', unpaid: '#DC2626',
};
...
const CPRetail = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const res = await dashboardAPI.getShopifyStore(dateRange?.start, dateRange?.end);
  ...
  const insights = hasLive
    ...
    : ['Run the Shopify pipeline to see live CP Store retail data.'];
                  ...
                  : (
                    <ResponsiveContainer width="100%" height={300}>
                      <BarChart data={data.monthly}>
                        ...
                        <Bar dataKey="revenue" name="Revenue" fill="#F97066" radius={[4,4,0,0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  )}
              </motion.div>
